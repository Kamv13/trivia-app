  ...
  return (
    <div className="card p-4 text-center">
      ...
      <div className="d-flex justify-content-center gap-3">
        <button className="btn btn-success" disabled={seleccion !== null} onClick={() => manejarSeleccion(true)}>Verdadero</button>
        <button className="btn btn-danger" disabled={seleccion !== null} onClick={() => manejarSeleccion(false)}>Falso</button>
      </div>
      {mensaje && <p className="mt-3">{mensaje}</p>}
      {seleccion !== null && !esUltima && (
        <button className="btn btn-secondary mt-3" onClick={onSiguiente}>Siguiente</button>
      )}
      {seleccion !== null && esUltima && (
        <Link href="/resultado" className="btn btn-primary mt-3">Ver resultados</Link>
      )}
    </div>
  );
}
